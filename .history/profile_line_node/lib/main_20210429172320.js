...
d3.json('../data/data.json', function (json) {
  ...
  function brush_brushEvent (d) {
    ...
    if (brushStartPoint != null) {
      ...
      if (xPosition > w - margin.right) {
        distance = (w - margin.right - brushStartPoint.x) / scale
        xPosition = w - margin.right
      } else if (xPosition < margin.left) {
        distance = (brushStartPoint.x - margin.left) / scale;
