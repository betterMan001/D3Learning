...
d3.json('../data/data.json', function (json) {
  ...
  /* brush 和zoom事件*/
  //brush
  const brush = d3.brush().extent([[margin.left, 0], [w - margin.right, h - margin.top]])
  ...
  function brush_brushEvent (d) {
    ...
    if (brushStartPoint != null) {
      ...
      selection.attr("width", posValue * scale)
        .attr("height", posValue)
        .attr('x', Math.min(xPosition, brushStartPoint.x))
        .attr("y", Math.min(xPosition, brushStartPoint.x))
