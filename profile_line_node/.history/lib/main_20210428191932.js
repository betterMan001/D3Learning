...
d3.json('../data/data.json', function (json) {
  ...
  /* brush 和zoom事件*/
  //brush
  const brush = d3.brush().extent([+margin.left, 0], [w - margin.left - margin.right, h - margin.top])
  ...
  d3.selectAll('button')
    .on("click", function () {
      ...
      if (id == "reset") {
        ...
        if (id == "brush") {
          d3.select(".brush_area rect").attr("display", "inherit")
        } else {
          d3.select(".brush_area rect").attr("display", "none")
