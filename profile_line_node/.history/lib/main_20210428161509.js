...
d3.json('../data/data.json', function (json) {
  ...
  function brush_brushEvent (d) {
    if (brushStartPoint !== null) {
      ...
      let xPosition = brushStartPoint.x + distance * scale * xCorMulti;//用y去算x的位置

      const oldDistance = distance;

      if (xPosition > w) {
        distance = (w - brushStartPoint.x) / scale;
        xPosition = w;
      } else if (xPosition < 0) {
      ...
      if (oldDistance !== distance) {
        //新算出来的距离都是正的，移动的方向信息是存在oldDistance
      ...
      selection.attr('w', posValue * scale).attr('h', posValue);
